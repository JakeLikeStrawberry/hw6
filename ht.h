...
{
    //insert into hash
    double m = double(tItem)/double(CAPACITIES[mIndex_]);
    if(m>=alpha_){
        resize();
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    if(mIndex_>=(sizeof(CAPACITIES)-1)){
        throw std::logic_error("none capacities");
    }
    mIndex_++;
    clearTotalProbes();
    std::vector<HashItem*> tableOld=this->table_;
    ...
    HASH_INDEX_T in = 0;
    while(in<CAPACITIES[mIndex_-1]){
        if(tableOld[in]!=nullptr){
            if(tableOld[in]->deleted){
                delete tableOld[in];
            }
            else{
                this->insert(tableOld[in]->item);
                delete tableOld[in];
            }
        }
        in++;
